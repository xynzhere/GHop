...
namespace GHop.Classes
{
    ...
    public class Main : BaseUnityPlugin
    {
        ...
        private float smoothForwardSpeed = 0f;
        private float smoothStrafeSpeed = 0f;
        ...
        private float handYOffset = 0.77f;
        ...
        private float strafeAnimTime = 0f;

        private GameObject rig;
        private GameObject gorilla;
        private Transform headRigOriginalParent;

        // Sex begins :freakymoneyface:
        private void Start()
        {
            instance = this;
            log = Logger;
            ...
            GTPlayer.Instance.gameObject.AddComponent<SpeedIndicator>();

            strafe = GTPlayer.Instance.gameObject.AddComponent<MovementSystem>();
            ...
            headRigOriginalParent = VRRig.LocalRig.head.rigTarget.transform.parent;
            Debug.Log($"[ghop] head rig parent on start: {headRigOriginalParent?.name ?? "NULL"}");
            ...
            GameObject crosshairCanvas = new GameObject("CrosshairCanvas");
            Canvas cc = crosshairCanvas.AddComponent<Canvas>();
            ...
            crosshairImg.sprite = Sprite.Create(
                CreateCircleTexture(32, 10f, 2f),
                new Rect(0, 0, 32, 32),
                new Vector2(0.5f, 0.5f)
            );

            RectTransform crt = crosshairImg.GetComponent<RectTransform>();
        ...
        private void HandleFreecam()
        {
            ...
            if (nHeld && !lastNKey)
            {
                ...
                else
                {
                    ...
                    if (headRigOriginalParent != null)
                        VRRig.LocalRig.head.rigTarget.transform.SetParent(headRigOriginalParent, true);
                    bodyYaw = GorillaTagger.Instance.headCollider.transform.eulerAngles.y;
                }
            }
            lastNKey = nHeld;
        ...
        // This part below might make you sick, i threw up 29 times writing and fixing this shit over and over again
        private void HandleAnims()
        {
            ...
            if (!freecamActive)
            {
                ...
                float angle = Mathf.DeltaAngle(bodyYaw, headYaw);

                float maxSoft = 45f;
                float maxHard = 60f;

                if (Mathf.Abs(angle) > maxHard)
                {
                    bodyYaw = headYaw - Mathf.Sign(angle) * maxHard;
                }
                else if (Mathf.Abs(angle) > maxSoft)
                {
                    float target = headYaw - Mathf.Sign(angle) * maxSoft;
                    bodyYaw = Mathf.LerpAngle(bodyYaw, target, Time.deltaTime * 15f);
                }
            ...
            {
                float impact = Mathf.Clamp01(Mathf.Abs(playerRb.linearVelocity.y) / 10f);
                if (impact > 0.05f)
                    landingSquashVelocity = -impact * 1.2f;
            }
            wasAirborne = !grounded;

            float squashStiffness = 12f;
            float squashDamping = 4f;
            landingSquashVelocity += (-squashStiffness * landingSquash - squashDamping * landingSquashVelocity) * Time.deltaTime;
            ...
            float walkBlend = Mathf.Clamp01(walkAnimSpeed / 1.6f);
            float idleBlend = 1f - walkBlend;
            float runBlend = Mathf.Clamp01((walkAnimSpeed - 4f) / 6f);
            ...
            float swingAmount = Mathf.Clamp(walkAnimSpeed * 0.03f, 0f, 0.22f);
            ...
            float rawForward = localVel.z / Mathf.Max(flatVel.magnitude, 0.001f);
            float rawStrafe = localVel.x / Mathf.Max(flatVel.magnitude, 0.001f);

            smoothForwardSpeed = Mathf.Lerp(smoothForwardSpeed, rawForward, Time.deltaTime * 10f);
            smoothStrafeSpeed = Mathf.Lerp(smoothStrafeSpeed, rawStrafe, Time.deltaTime * 40f);

            float forwardSwing = swingAmount * smoothForwardSpeed;
            float leftSwing = Mathf.Sin(walkAnimTime) * forwardSwing;
            float rightSwing = Mathf.Sin(walkAnimTime + Mathf.PI) * forwardSwing;

            strafeAnimTime += Time.deltaTime * walkAnimSpeed * 1.2f;

            float strafeAmount = swingAmount * Mathf.Abs(smoothStrafeSpeed) * 0.5f;
            float strafeDir = Mathf.Sign(rawStrafe);

            float leftCircleX = Mathf.Sin(strafeAnimTime) * strafeAmount * strafeDir;
            float leftCircleY = Mathf.Cos(strafeAnimTime) * strafeAmount * 0.5f;
            float rightCircleX = Mathf.Sin(strafeAnimTime + Mathf.PI) * strafeAmount * strafeDir;
            float rightCircleY = Mathf.Cos(strafeAnimTime + Mathf.PI) * strafeAmount * 0.5f;

            float bobAmount = swingAmount * 0.4f;
            float leftBob = Mathf.Pow(Mathf.Abs(Mathf.Sin(walkAnimTime)), 0.25f) * bobAmount;
            float rightBob = Mathf.Pow(Mathf.Abs(Mathf.Sin(walkAnimTime + Mathf.PI)), 0.25f) * bobAmount;

            float swayAmount = swingAmount * 0.05f;
            float sway = Mathf.Sin(walkAnimTime * 1f) * swayAmount;

            float headBob = Mathf.Abs(Mathf.Sin(walkAnimTime)) * 0.008f * walkBlend;
            float headTiltX = Mathf.Sin(walkAnimTime) * 1.5f * walkBlend - runBlend * 4f;
            ...
            leftY += runBlend * 0.02f;
            rightY += runBlend * 0.02f;

            leftY += landingSquash * 0.3f;
            rightY += landingSquash * 0.3f;
            ...
            VRRig.LocalRig.transform.position = GorillaTagger.Instance.bodyCollider.transform.position
                + new Vector3(0f, breatheOffset + headBob + landingSquash * 0.3f, 0f);
            ...
            VRRig.LocalRig.leftHand.rigTarget.transform.position = leftHandBase
                + VRRig.LocalRig.transform.forward * leftSwing
                + VRRig.LocalRig.transform.right * leftCircleX
                - new Vector3(0f, leftY + breatheOffset - leftCircleY * walkBlend, 0f);

            VRRig.LocalRig.rightHand.rigTarget.transform.position = rightHandBase
                + VRRig.LocalRig.transform.forward * rightSwing
                + VRRig.LocalRig.transform.right * rightCircleX
                - new Vector3(0f, rightY + breatheOffset - rightCircleY * walkBlend, 0f);

            float leftRotX = swingAmount > 0 ? leftSwing / swingAmount * 21f * walkBlend : 0f;
            float rightRotX = swingAmount > 0 ? rightSwing / swingAmount * 21f * walkBlend : 0f;
